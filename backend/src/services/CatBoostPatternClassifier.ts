/**
 * CatBoost Pattern Classifier Service
 *
 * Provides CatBoost-based pattern classification by calling the Python microservice
 * Bayesian framework + CatBoost ML implementation
 *
 * Model Performance (3-round Bootstrap validation):
 * - Accuracy: 93.0% (+/-1.6%)
...
export interface CatBoostPrediction {
  pattern: Pattern;
  probability: number;
...
/**
 * CatBoost Pattern Classifier
 * Communicates with Python microservice for predictions
 *
 * Uses CatBoost gradient boosting which provides:
 * - Better handling of imbalanced classes
 * - Ordered boosting to reduce overfitting
 * - Native support for categorical features
 */
class CatBoostPatternClassifier {
  private apiClient: AxiosInstance;
  private serviceUrl: string = process.env.ML_SERVICE_URL || 'http://localhost:5002';
   ...
   * Silently fails if service not available - will retry on first use
   */
  private async initialize(): Promise<void> {
    try {
      ...
      console.log('CatBoost Classifier initialized successfully');
    } catch (error) {
  ...
  /**
   * Convert behavioral signals to ML input format
   */
  ...
  /**
   * Predict pattern from behavioral signals using CatBoost
   */
  async predictPattern(signals: BehavioralSignals): Promise<PatternEstimate> {
    ...
    try {
      ...
      const prediction: CatBoostPrediction = response.data;
      ...
      console.error('CatBoost prediction error:', error.message);
      ...
      return {
        ...
        needMoreData: true,
        evidence: ['Error in ML prediction, using default distribution'],
      };
    }
  }
  ...
  async batchPredict(signalsList: BehavioralSignals[]): Promise<PatternEstimate[]> {
    try {
      ...
      return response.data.results.map((pred: CatBoostPrediction) => ({
...
export default new CatBoostPatternClassifier();
