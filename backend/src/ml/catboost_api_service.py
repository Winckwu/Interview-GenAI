"""
CatBoost Pattern Classifier API Service
========================================

Flask microservice that provides CatBoost-based pattern classification
Listens on port 5002 and exposes REST endpoints

Based on model comparison results:
- CatBoost: 93.0% (+/-1.6%) Bootstrap accuracy
...
def load_model_artifacts():
    ...
    try:
        ...
        with open(MODEL_DIR / 'catboost_scaler.pkl', 'rb') as f:
        ...
        return catboost_model, scaler, mapping, feature_names
...
CATBOOST_MODEL, SCALER, PATTERN_MAPPING, FEATURE_NAMES = load_model_artifacts()

if CATBOOST_MODEL is None:
    print("Warning: CatBoost model not loaded. Service may not work correctly.")
else:
...
def health_check():
    ...
    return jsonify({
        'status': 'ok',
        'service': 'catboost-classifier',
        'model_loaded': CATBOOST_MODEL is not None
    })
...
def predict():
    ...
    try:
        ...
        # Get prediction
        prediction = CATBOOST_MODEL.predict(X_scaled)[0]
        ...
        probabilities = CATBOOST_MODEL.predict_proba(X_scaled)[0]
...
def model_info():
    ...
    return jsonify({
        ...
        'model_accuracy': '93.0% (+/-1.6%)',
        'pattern_f_recall': '97.9%',
        'classes': [PATTERN_MAPPING['id_to_pattern'][str(i)] for i in range(6)]
    })


if __name__ == '__main__':
    print("Starting CatBoost Pattern Classifier API Service")
