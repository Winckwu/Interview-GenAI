"""
SVM Pattern Classifier - High Precision Training

Uses the original trainModel.py parameters (C=1.0) to achieve 77%+ accuracy
instead of 59% achieved with C=10.0
"""
...
def train_svm_model(X_train: np.ndarray, y_train: np.ndarray,
    ...
    """
    Train SVM classifier with ORIGINAL high-precision parameters

    Original trainModel.py parameters:
    - kernel: 'rbf'
    - C: 1.0  (NOT 10.0! This was causing overfitting)
    - gamma: 'scale'
    - probability: True
    - random_state: 42
    """

    print("\n🤖 Training SVM Classifier (High Precision Parameters)...")
    print("   Using original parameters from trainModel.py:")
    print("   - kernel: 'rbf'")
    print("   - C: 1.0 (original, NOT 10.0)")
    print("   - gamma: 'scale'")
    ...
    # Train SVM with ORIGINAL parameters (C=1.0, not C=10.0)
    svm = SVC(
        kernel='rbf',
        C=1.0,              # ⭐ ORIGINAL: 1.0 (NOT 10.0)
        gamma='scale',      # Original
        probability=True,   # Original
...
def main():
    print("="*80)
    print("🚀 SVM Pattern Classifier - HIGH PRECISION Training")
    print("="*80)
    print("\n⚠️  Using ORIGINAL trainModel.py parameters (C=1.0)")
    print("    Previous attempt used C=10.0 which caused overfitting")
    print("    Expected accuracy: ~77% (vs 59% with C=10.0)")

    # Find best available dataset (priority: largest dataset)
    current_dir = Path(__file__).parent
    csv_paths = [
        current_dir / 'augmented_training_data.csv',  # 109 samples (最大)
        current_dir / 'enhanced_training_data.csv',   # 109 samples
        current_dir / 'hybrid_training_data.csv',     # 79 samples
        current_dir / 'training_data.csv',            # 49 samples
    ]
    ...
    # Train SVM with CORRECT parameters
    ...
    print("\n" + "="*80)
    print("✅ HIGH PRECISION SVM Training Complete!")
    print("="*80)
    print(f"\n📈 Summary:")
    ...
    print(f"\n🎯 Expected improvement from 59% → ~77% accuracy")
